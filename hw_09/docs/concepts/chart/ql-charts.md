---
title: "QL-чарты в {{ datalens-full-name }}. Обзор"
description: "Из статьи вы узнаете, что такое QL-чарты в {{ datalens-full-name }} и познакомитесь с их особенностями."
---
...
* **{{ prometheus-name }}**. Позволяют построить график по данным {{ prometheus-name }}{% if audience == "internal" %} (см. [пример]({{ link-datalens-main }}/ql/ssvptrd5tqh0k)){% endif %}. Поверх этого источника не может быть построен датасет, работать с ним можно только с помощью QL-чартов.

  {% include [datalens-prometheus-ch-example](../../../_includes/datalens/datalens-prometheus-ch-example.md) %}

* **{{ monitoring-short-name }}**. Позволяют построить график по данным {{ monitoring-full-name }}{% if audience == "internal" %} (см. [пример]({{ link-datalens-main }}/ql/99c6irbpsmam1)){% endif %}. Поверх этого источника не может быть построен датасет, работать с ним можно только с помощью QL-чартов.{% if audience == "internal" %} Также можно [экспортировать](../../operations/chart/export-from-monitoring.md) в {{ datalens-short-name }} готовый график из {{ monitoring-short-name }}. Его можно отредактировать, сохранить и разместить на дашборде.{% endif %}
