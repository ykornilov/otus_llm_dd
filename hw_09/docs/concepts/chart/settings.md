---
title: "Настройки чарта в {{ datalens-full-name }}"
description: "В этой статье вы узнаете о настройках чарта в {{ datalens-full-name }}, а также узнаете, как отменить и восстановить изменения в чартах."
---

# Настройки чарта в {{ datalens-full-name }}
...
{% if audience == "internal" %}|| Фид комментариев | Id канала комментариев из другого чарта. Используется для автоматического дублирования общих для разных чартов комментариев. Настройка доступна только для чартов с временными рядами (у которых в секции **X** или **Y** используется измерение с типом `Дата` и `Дата и время`).<br/>Подробнее см. [инструкцию](../../operations/chart/use-comments.md). ||{% endif %}
...
|| Пагинация | Разбивает таблицу на страницы. Недоступно, если отображается только одна страница или нет данных.<br/><br/>Доступные значения:
...
{% if product != "dltech" %}|| Накопление | Задает в [диаграмме с областями](../../visualization-ref/area-chart.md#stacking) отображение данных с накоплением или отдельно для каждой категории.<br/><br/>Доступные значения:

* **Вкл** — отображать с накоплением.
* **Выкл** — отображать без накопления. ||{% endif %}
{% if product == "yandex-cloud" %}|| Центр | Задает в [Картах](../../visualization-ref/map-chart.md) координаты центра по умолчанию.<br/><br/>Доступные значения:
...
* `Нет` — без разметки.
* `HTML` — разметка HTML. Доступно только для полей с типом `Строка`. Например:

  ```html
  '<span style="font-size: 16px;">' +
  str([Продукт - категория])
  + '</span>'
  ```
...
{% if product == "yandex-cloud" %}  || [Карты](../../visualization-ref/map-chart.md) | [Тултип](#map-settings) ||{% endif %}
  || [Линейная](../../visualization-ref/line-chart.md), [Столбчатая](../../visualization-ref/column-chart.md), [Линейчатая](../../visualization-ref/bar-chart.md), [Круговая](../../visualization-ref/pie-chart.md){% if product != "dltech" %}, [Кольцевая](../../visualization-ref/ring-chart.md) диаграммы и [Диаграмма c областями](../../visualization-ref/area-chart.md){% else %} и [Кольцевая](../../visualization-ref/ring-chart.md) диаграммы{% endif %} | [Подписи](#sign) ||
  |#
...
|| Форматирование оси | Задает форматирование подписей на оси с числовыми значениями, со значениями даты или даты и времени.<br/><br/>Доступные значения:

* **Авто** — форматирование по умолчанию.
* **По первому полю на оси X** — отображать подписи к оси X с форматированием, указанным в настройке **Формат** для первого поля в секции **X**. Доступно в настройках оси X для числовых значений.
* **По первому полю на оси Y** — отображать подписи к оси Y с форматированием, указанным в настройке **Формат** для первого поля в секции **Y**. Доступно в настройках оси Y для числовых значений.
* **Вручную** — позволяет задать параметры форматирования, соответствующие типу данных поля в секции оси. Доступно для числовых значений, значений даты или даты и времени. ||
...
{% if product != "dltech" %}* **Использовать предыдущее** — заменять пустые значения в чарте на значение предыдущей точки на оси. Доступно в настройках оси Y для [накопительной диаграммы](../../visualization-ref/area-chart.md). {% endif %}||
...
Для [древовидной диаграммы](../../visualization-ref/tree-chart.md), [таблицы](../../visualization-ref/table-chart.md) (в том числе [сводной](../../visualization-ref/pivot-table-chart.md)){% if product == "yandex-cloud" %} и [карты](../../visualization-ref/map-chart.md){% endif %} доступны следующие настройки:
...
{% if audience != "internal" %}Вы можете [создать палитры](../../operations/chart/create-palette.md) цветов и использовать их в своих чартах.{% endif %}
...
{% if product != "dltech" and product != "dlent" %}
